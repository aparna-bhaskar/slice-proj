...
static elem_t cpu_output[REAL_K_I][REAL_K_J] row_align(1);
static elem_t gemmini_output[REAL_K_I][REAL_K_J] row_align(1);


static void run_cpu_k_projection(void) {
    for (size_t i = 0; i < REAL_K_I; i++) {
        for (size_t j = 0; j < REAL_K_J; j++) {
            acc_t sum = 0;

            for (size_t k = 0; k < REAL_K_K; k++) {
                sum +=
                    (acc_t)REAL_K_X[i][k]
                    * (acc_t)REAL_K_W_TRANSPOSED[k][j];
            }

            cpu_output[i][j] =
                (elem_t)ACC_SCALE(sum, REAL_K_C_SCALE);
        }
    }
}
...
static int compare_matrices(
    const char *name,
    const elem_t left[REAL_K_I][REAL_K_J],
    const elem_t right[REAL_K_I][REAL_K_J]
) {
...
int main(void) {
    ...
    printf("X shape: [%d, %d]\n", REAL_K_I, REAL_K_K);
    printf("WK^T shape: [%d, %d]\n", REAL_K_K, REAL_K_J);
    printf("K shape: [%d, %d]\n", REAL_K_I, REAL_K_J);
    //printf("Gemmini C scale: %f\n", (double)REAL_K_C_SCALE);

    printf("\nStarting scalar CPU K projection\n");

    unsigned long cpu_start = read_cycles();

    run_cpu_k_projection();
    ...
    if (
        ...
        printf("\nFAIL: real K projection did not match.\n");
        return 1;
    }

    printf(
        "\nPASS: real layer-0 K projection matched "
